...
internal fun Document.toDatabaseDto() = DatabaseDto(
    name = getString(NAME),
    sizeOnDisk = getLong(SIZE_ON_DISK),
    empty = getBoolean(EMPTY)
)

internal fun Document.toCollectionDto() = CollectionDto(
    name = getString(NAME),
    type = getString(TYPE),
    options = get(OPTIONS, Document::class.java),
    info = get(INFO, Document::class.java).toCollectionInfo(),
    idIndex = get(ID_INDEX, Document::class.java).toCollectionIndex()
)

internal fun Document.toCollectionInfo() = CollectionDto.Info(
    readOnly = getBoolean(READ_ONLY),
    uuid = get(UUID, Binary::class.java).toUuid()
)

internal fun Document.toCollectionIndex() = CollectionDto.Index(
    v = getInteger(V),
    name = getString(NAME),
