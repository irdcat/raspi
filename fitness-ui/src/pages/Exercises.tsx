...
export const Exercises = () => {
  ...
  useAsyncEffect(async () => {
    await ExercisesApi.get().then((exercises) => setExerciseList(exercises));
  }, []);
  ...
  const onAddExercise = (exercise: Exercise) => {
    ExercisesApi.add(exercise).then((e) => setExerciseList([...exerciseList, e]));
  };

  const onEditExercise = (exercise: Exercise) => {
    ExercisesApi.update(exercise.id, exercise).then((updated) => {
  ...
  const onDeleteExercise = (id: string) => {
    ExercisesApi.delete(id).then((deleted) => setExerciseList(exerciseList.filter((e) => e.id !== deleted.id)));
  };
          ...
          <TableBody sx={{ overflowY: 'scroll' }}>
            {exerciseList.map((exercise) => (
              <TableRow key={exercise.id} sx={{ '&:lastchild td, &:last-child th': { border: 0 } }}>
                ...
                <TableCell>{exercise.isBodyWeight ? 'Yes' : 'No'}</TableCell>
                <TableCell align="right">
                  <ButtonGroup variant="outlined">
                    <Button onClick={() => handleSummaryClick(exercise.id)} color="success">
                      Summary
                    </Button>
                    <ButtonActivatedDialog
                      title="Edit Exercise"
