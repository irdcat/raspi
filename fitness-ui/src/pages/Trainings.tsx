...
export const Trainings = () => {
  ...
  const onEditTraining = (training: Training) => {
    TrainingsApi.update(training.id, training).then((updated) => {
  ...
  const onDeleteTraining = (id: string) => {
    TrainingsApi.delete(id).then((deleted) => setTrainingList(trainingList.filter((t) => t.id !== deleted.id)));
  };
            ...
            <TrainingForm
              onSubmit={(formData: TrainingFormData) => {
                onAddTraining({
                  ...
                  exercises: formData.exercises.map((trainingExercise, index) => ({
                    order: index,
                    exerciseId: trainingExercise.exerciseId,
                    sets: trainingExercise.sets.map((trainingExerciseSet) => ({
          ...
          <TableBody>
            {trainingList.map((training) => (
              <TableRow key={training.id} sx={{ '&:lastchild td, &:last-child th': { border: 0 } }}>
                <TableCell>{training.date.toDateString()}</TableCell>
                <TableCell>{training.bodyWeight}</TableCell>
                <TableCell align="right">
                  <ButtonGroup variant="outlined">
                    <Button onClick={() => onClickSummary(training.id)} color="success">
                      Summary
                    </Button>
                    <ButtonActivatedDialog
                      title="Edit Training"
