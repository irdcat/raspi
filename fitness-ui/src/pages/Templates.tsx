...
export const Templates = () => {
  ...
  useAsyncEffect(async () => {
    await TemplatesApi.get().then((templates) => setTemplateList(templates));
  }, []);
  ...
  const onDeleteTemplate = (id: string) => {
    TemplatesApi.delete(id).then((deleted) => setTemplateList(templateList.filter((t) => t.id !== deleted.id)));
  };
          ...
          <TableBody>
            {templateList.map((template) => (
              <TableRow key={template.id} sx={{ '&:lastChild td, &:last-child th': { border: 0 } }}>
                ...
                <TableCell>{template.groupName}</TableCell>
                <TableCell>{template.description}</TableCell>
                <TableCell align="right">
                  <ButtonGroup variant="outlined">
                    <Button onClick={() => onClickDetails(template.id)} color="success">
                      Details
                    </Button>
                    <ButtonActivatedDialog
                      title="Edit Template"
                      ...
                      {(close) => (
                        <TemplateForm
                          ...
                          initialValues={{
                            ...
                            exerciseIds: template.exerciseIds.map((e) => ({
                              id: e,
                            })),
                          }}
                        />
