            ...
            <TableCell>
                <FormInputText
                    ...
                    register={props.registerFunction(`${props.name}.reps`, {
                        ...
                        validate: { positive: (v: number) => parseInt(v.toString()) >= 0 || "Value must be non-negative integer" }
                    })}
                    label="Repetitions"/>
            </TableCell>
            <TableCell>
                <FormInputText
                    ...
                    register={props.registerFunction(`${props.name}.weight`, {
                        required: { value: true, message: "Field is required" },
                        validate: { positive: (v: number) => parseFloat(v.toString()) >= 0 || "Value must be non-negative real number" }
                    })}
                    label="Weight"/>
            </TableCell>
